...
当前主路径为 sub2api 用户登录、注册和 2FA，浏览器仅持有 HttpOnly Cookie session，业务数据按用户 owner 隔离。项目仍保留管理员兼容会话用于本地开发、旧测试和设置管理；自托管部署需要 PostgreSQL、Redis、后端 API、Dramatiq worker、Web 前端，以及可用的 sub2api 与文本/图片模型供应商。

## 功能概览

### 商品/工作台

- sub2api 邮箱/密码登录、注册和 2FA，基于 HttpOnly Cookie session 访问后台 API。
...
当前多用户能力依赖外部 sub2api 作为身份、注册、2FA、余额/用量和用户 API key 真源；ProductFlow 不内置托管账号体系或本地计费。暂不提供团队权限、复杂 RBAC、自动投放/自动上架、视频生成、Kubernetes/Helm/发布版容器镜像等生产编排包。仓库内 Docker Compose 自托管路径已可用。
...
- `ADMIN_ACCESS_KEY`：管理员兼容会话和本地开发入口使用的密钥；密钥本身只从环境变量读取，不写入数据库。
- `SETTINGS_ACCESS_TOKEN`：配置页二次解锁令牌，必须与管理员兼容密钥分开。
...
Web 默认入口：`http://127.0.0.1:29281`（改过端口时使用 `.env` 中的 `WEB_PORT`）。配置 sub2api 后，使用 sub2api 邮箱/密码登录、注册或完成 2FA；未配置真实 sub2api 时，可使用管理员兼容接口完成开发和 mock provider 验证。Web 镜像提供 Vite build 后的静态资源，nginx 将同源 `/api/*` 请求反向代理到 `productflow-backend:29280`。
...
- `SETTINGS_ACCESS_TOKEN`：配置页二次解锁令牌，必须与管理员兼容密钥分开。
- `SESSION_SECRET`：签名 session cookie 的长随机字符串。
- `CREDENTIAL_VAULT_KEY`：加密 sub2api access token、临时 token 和 API key 的稳定随机密钥。
...
打开 Web 页面后使用 sub2api 账号登录；未配置真实 sub2api 时，可通过管理员兼容路径验证本地 mock 流程。顶部导航提供 **商品/工作台**、**文/图生图**、**画廊**、**帮助**、**账号** 和管理员可见的 **配置**。
...
ProductFlow 把文本和图片能力分开配置。基础设施配置（数据库、Redis、session、sub2api 地址、凭据加密密钥、管理员兼容密钥）仍然只从环境变量读取；业务配置可在前端 `/settings` 页面写入数据库并覆盖环境变量默认值。

登录门禁 `admin_access_required` 默认开启。普通工作台和私有 API 需要有效的 `productflow_session`；主路径由 sub2api 登录/注册/2FA 创建用户会话，管理员兼容接口用于本地开发、旧测试和受控管理场景。二次解锁 `/settings` 后可关闭该开关，让普通工作台/API 使用 `dev:admin` 兼容会话。`SETTINGS_ACCESS_TOKEN` 始终独立保护配置页读取和写入。
