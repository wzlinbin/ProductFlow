...
1. Env-only 基础设施配置：`DATABASE_URL`、`REDIS_URL`、`SESSION_SECRET`、`CREDENTIAL_VAULT_KEY`、`SUB2API_AUTH_BASE_URL`、`SUB2API_PROVIDER_BASE_URL`、`ADMIN_ACCESS_KEY`、`SETTINGS_ACCESS_TOKEN` 等。这些配置在应用访问数据库前就必须可用，或用于保护凭据、管理员兼容入口和配置页二次解锁，因此不支持运行时 DB 覆盖。
2. 运行时业务配置：provider、模型、图片尺寸、上传限制、任务重试、全局生成并发上限、海报模式、提示词模板、登录门禁开关、业务删除开关等。它们可由 `.env` / `.env.dev` 提供默认值，也可在管理员登录并二次解锁设置页后通过 `/api/settings` 写入 `app_settings` 并覆盖。

Secret 类配置在 API 响应中不回显已有值。

登录门禁开关 `admin_access_required` 默认开启；开启时私有 API 通过 `require_user` 要求有效 `productflow_session`，主路径由 sub2api 登录/注册/2FA 创建会话，管理员兼容接口仍会校验 `ADMIN_ACCESS_KEY`。关闭时普通工作台和私有 API 使用 `dev:admin` 兼容会话，`GET /api/auth/session` 返回 `authenticated=true` 和 `access_required=false`；但 `/api/settings` 的完整配置读取/写入仍必须是管理员会话并先通过独立的 `SETTINGS_ACCESS_TOKEN` 解锁。
...
当前安全模型是“sub2api 身份 + 自托管业务数据”：

- 登录、注册、2FA、余额/用量和用户 API key 以 sub2api 为真源；ProductFlow 只保存本地 HttpOnly Cookie session 和服务端加密后的 sub2api token/key。
- 业务数据按 `owner_id` 隔离；跨用户访问按未找到处理，避免暴露资源存在性。
- `ADMIN_ACCESS_KEY` 只从环境变量读取，不进入数据库配置，仅用于管理员兼容会话、本地开发和旧测试；登录门禁可通过 `admin_access_required` 运行时开关关闭，但默认保持开启。
- 配置页使用管理员权限和独立 `SETTINGS_ACCESS_TOKEN` 二次解锁；session 只保存已解锁时间，不保存令牌明文。关闭登录门禁不会关闭这个二次解锁。
- `CREDENTIAL_VAULT_KEY` 用于加密 sub2api access token、2FA 临时 token 和用户 API key，生产环境必须显式设置并保持稳定。
- Session cookie 由 `SESSION_SECRET` 保护，认证 cookie 名为 `productflow_session`。
- CORS 由 `BACKEND_CORS_ORIGINS` 控制。
- 上传文件有 MIME、大小、像素和数量限制。
- Provider/API key 不在接口中回显；真实用户生成优先使用用户绑定的 sub2api credential。

当前不提供团队权限、复杂 RBAC、审计后台、生产 WAF 配置或替代 sub2api 的本地账号/计费体系。
