...
非目标用户：需要复杂 RBAC、团队审计、内置支付结算、素材投放平台、托管账号体系，或不希望依赖 sub2api 作为身份与计费真源的团队。

## 3. 当前核心场景

### 3.1 商品素材链路

1. 使用 sub2api 账号登录、注册，必要时完成 2FA。
...
- `AuthSession` / `AuthLoginChallenge` / `UserProviderCredential`：本地 session、2FA 临时挑战和服务端加密的用户 provider credential。

## 5. 当前页面

前端当前实现的页面：

- `/login`：sub2api 邮箱/密码登录、注册和 2FA。
- `/products`：商品列表。
- `/products/new`：创建商品。
- `/products/:productId`：商品详情、文案/海报主链路、历史、DAG 工作流。
- `/gallery`：生成图画廊。
- `/help`：产品内帮助页。
- `/account`：sub2api 用户信息、API key 状态、余额和用量。
- `/settings`：管理员专用的 provider、模型、上传限制、任务重试等业务配置。
...
1. 用 sub2api 账号完成登录/注册/2FA 主路径，并用本地 `mock` provider 在无外部模型密钥时跑通商品链路。
...
10. 通过管理员专用 `/settings` 保存业务配置覆盖，并避免在 API 响应中回显 secret 值。
...
- 团队权限、复杂 RBAC、审计后台。
- 替代 sub2api 的本地账号体系、托管模型密钥或内置计费系统。
- 自动投放、自动上架、店铺授权。
- 视频生成链路。
