...
- sub2api 用户登录/注册/2FA、owner 隔离和管理员兼容会话，而不是完整多租户 SaaS。
- ProductFlow workbench、连续生图、画廊、账号页、管理员设置页和运行时配置。
- 三类异步执行入口和轻量 status polling。
- Docker Compose 自托管路径和本地开发路径。
- 当前明确不包含团队权限、复杂 RBAC、内置支付、自动投放、对象存储、Helm、发布版镜像或替代 sub2api 的本地账号/计费体系。
...
3. **保持 docs 的 current/future 分层**  
   当前事实继续放在 README、PRD、ARCHITECTURE、USER_GUIDE 和本复审；对象存储、SSE/WebSocket、Helm、团队权限、复杂 RBAC 等只放在 roadmap 的未来方向或暂不计划，不写成已实现能力。
