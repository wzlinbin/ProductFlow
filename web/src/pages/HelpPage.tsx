...
const DOC_PAGES_EN: DocPage[] = [
  ...
  },
  {
    ...
    description: "The product workbench is the core operating surface. The canvas is in the center, with inspectors and supporting panels on the right.",
    category: "Canvas workbench",
    icon: GitBranch,
    sections: [
      { id: "layout", title: "Layout", blocks: [{ type: "table", headers: ["Area", "Description"], rows: [["Canvas", "Shows product, reference image, copy, and image generation nodes plus their edges."], ["Details", "Edits the selected node configuration and output."], ["Runs", "Shows workflow run history, failure reasons, and retryable runs."], ["Images", "Shows product assets, generated images, and images that can fill reference nodes."], ["Templates", "Inserts built-in scene templates or manages user-saved templates."]] }] },
  ...
  },
  {
    ...
    icon: Box,
    sections: [
      { id: "canvas-controls", title: "Canvas controls", blocks: [{ type: "table", headers: ["Action", "Description"], rows: [["Zoom", "Use the mouse wheel, or the bottom-right - / percentage / + controls."], ["Pan", "Drag an empty canvas area with the left mouse button when no modifier is pressed. Dragging nodes, clicking controls, uploading, and drawing edges do not pan the canvas."], ["Move one node", "Drag the node body or title area. The position is saved after release."], ["Move multiple nodes", "Select multiple nodes, then drag any selected node to move the group."], ["Create an edge", "Drag from the output handle on the right of a node to the input handle on the left of the target node."], ["Select one node", "Click the node body or the input handle. The right panel switches to Details."], ["Add or remove from multi-select", "Ctrl-click a node. On macOS, Cmd-click also works. Shift-click also toggles selection."], ["Box select", "Hold Shift, drag from an empty canvas area, and release to select nodes inside the box."]] }] },
  ...
  },
  {
    ...
    icon: Sparkles,
    sections: [
      { id: "layout", title: "Page layout", blocks: [{ type: "table", headers: ["Area", "Use"], rows: [["Left session list", "Create, select, rename, or delete image chat sessions. Each session keeps its own history, references, and generation tasks."], ["Center result area", "Shows the selected generated candidate, running placeholders, failed state, download button, and Send to gallery button."], ["Bottom history", "Shows results by branch. Clicking a completed image selects it as the current result and base image for the next round."], ["Right generation settings", "Manages linked product, save-to-product actions, session references, image description, size, candidate count, and advanced image tool parameters."]] }] },
      { id: "create-session", title: "Create and select sessions", blocks: [{ type: "steps", items: ["Open Image chat from the top navigation.", "Click New in the left session area.", "If you entered from a product detail page, the page uses product-linked mode. From the global entry, you can generate freely or select a target product on the right.", "Click any session card on the left to switch sessions. Cards show the latest thumbnail, round count, and update time.", "To rename a session, open Generation settings, click Rename, enter a name, and click save."] }, { type: "callout", title: "Sessions and products are different objects", text: "An image chat session can be linked to a product or used freely. The current candidate is written back to the product asset library only when you click actions such as Add reference image, Save as reference, or Set as product main image." }] },
    ],
  },
  {
    slug: "image-chat-references",
  ...
  },
  {
    ...
    icon: Sparkles,
    sections: [
      { id: "generation-settings", title: "Fields", blocks: [{ type: "table", headers: ["Setting", "Description"], rows: [["Image description", "The actual user requirement submitted for this round. State subject, what to preserve, what to change, background, composition, lighting, and purpose."], ["Size", "Choose common 1K / 2K / 4K presets or enter a custom width and height. Values are calibrated against the backend maximum single-edge limit before submission."], ["Candidate count", "Controls how many candidates are created for this round. Multiple candidates appear as placeholders in history and are replaced individually when complete."], ["Generation / Advanced", "Regular size, count, and description are in the basic settings. Provider tool parameters are in Advanced."], ["Image tool parameters", "Only fields enabled in Settings under available tool fields are visible, such as quality, format, background, and input fidelity."]] }] },
  ...
  },
  {
    ...
    sections: [
      ...
      { id: "mobile-layout", title: "Mobile layout", blocks: [{ type: "paragraph", text: "On small screens, image chat becomes a single-column layout. Sections appear in order: sessions, current result, history, generation settings, and references. The desktop three-column layout is not preserved." }] },
    ],
  },
  {
    slug: "gallery",
    ...
    icon: GalleryHorizontalEnd,
    sections: [
      { id: "save-to-gallery", title: "Save to gallery", blocks: [{ type: "paragraph", text: "Image chat results can be saved to the gallery. Each gallery entry keeps source session, linked product, prompt, size, model, and download access." }, { type: "list", items: ["Useful for backgrounds or compositions that are not yet attached to a product but may be reused later.", "Useful for collecting candidates for others to review.", "Useful for saving good parameter-exploration results that are not the current final draft."] }] },
    ],
  },
  {
    slug: "settings",
